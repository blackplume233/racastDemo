...
int Sample_TempObstacles::rasterizeTileLayers(
	...
	RasterizationContext rasterization_context;
	...
	rasterization_context.solid = rcAllocHeightfield();
	if (!rasterization_context.solid) {
		m_ctx->log(RC_LOG_ERROR, "buildNavigation: Out of memory 'solid'.");
		return 0;
	}
	if (!rcCreateHeightfield(m_ctx, *rasterization_context.solid, tcfg.width, tcfg.height, tcfg.bmin, tcfg.bmax, tcfg.cs, tcfg.ch)) {
	...
	rasterization_context.triareas = new unsigned char[chunkyMesh->maxTrisPerChunk];
	if (!rasterization_context.triareas) {
	...
	for (int i = 0; i < ncid; ++i) {
		...
		memset(rasterization_context.triareas, 0, ntris * sizeof(unsigned char));
		rcMarkWalkableTriangles(m_ctx, tcfg.walkableSlopeAngle,
			verts, nverts, tris, ntris, rasterization_context.triareas);

		if (!rcRasterizeTriangles(m_ctx, verts, nverts, tris, rasterization_context.triareas, ntris, *rasterization_context.solid, tcfg.walkableClimb))
			return 0;
	}
	...
	if (m_filterLowHangingObstacles)
		rcFilterLowHangingWalkableObstacles(m_ctx, tcfg.walkableClimb, *rasterization_context.solid);
	if (m_filterLedgeSpans)
		rcFilterLedgeSpans(m_ctx, tcfg.walkableHeight, tcfg.walkableClimb, *rasterization_context.solid);
	if (m_filterWalkableLowHeightSpans)
		rcFilterWalkableLowHeightSpans(m_ctx, tcfg.walkableHeight, *rasterization_context.solid);


	rasterization_context.chf = rcAllocCompactHeightfield();
	if (!rasterization_context.chf) {
		m_ctx->log(RC_LOG_ERROR, "buildNavigation: Out of memory 'chf'.");
		return 0;
	}
	if (!rcBuildCompactHeightfield(m_ctx, tcfg.walkableHeight, tcfg.walkableClimb, *rasterization_context.solid, *rasterization_context.chf)) {
		m_ctx->log(RC_LOG_ERROR, "buildNavigation: Could not build compact data.");
		return 0;
	}

	// Erode the walkable area by agent radius.
	if (!rcErodeWalkableArea(m_ctx, tcfg.walkableRadius, *rasterization_context.chf)) {
	...
	for (int i = 0; i < m_geom->getConvexVolumeCount(); ++i) {
		rcMarkConvexPolyArea(m_ctx, vols[i].verts, vols[i].nverts,
			...
			(unsigned char)vols[i].area, *rasterization_context.chf);
	}

	rasterization_context.lset = rcAllocHeightfieldLayerSet();
	if (!rasterization_context.lset) {
		m_ctx->log(RC_LOG_ERROR, "buildNavigation: Out of memory 'lset'.");
		return 0;
	}
	if (!rcBuildHeightfieldLayers(m_ctx, *rasterization_context.chf, tcfg.borderSize, tcfg.walkableHeight, *rasterization_context.lset)) {
		m_ctx->log(RC_LOG_ERROR, "buildNavigation: Could not build heighfield layers.");
		return 0;
	}

	rasterization_context.ntiles = 0;
	for (int i = 0; i < rcMin(rasterization_context.lset->nlayers, MAX_LAYERS); ++i) {
		TileCacheData *tile = &rasterization_context.tiles[rasterization_context.ntiles++];
		const rcHeightfieldLayer *layer = &rasterization_context.lset->layers[i];
	...
	int n = 0;
	for (int i = 0; i < rcMin(rasterization_context.ntiles, maxTiles); ++i) {
		tiles[n++] = rasterization_context.tiles[i];
		rasterization_context.tiles[i].data = 0;
		rasterization_context.tiles[i].dataSize = 0;
	}
...
public:
	...
	virtual void handleMenu() {
		...
		imguiValue(std::to_string(time_diff).c_str());
		imguiValue(std::to_string(p).c_str());
		p++;
	}
	...
	virtual void handleClick(const float *s, const float *p, bool shift) {
		...
		std::chrono::duration<double> d_diff = _end_time - _start_time;
	}
...
void Sample_TempObstacles::handleUpdate(const float dt) {
	...
	auto _end_time = std::chrono::steady_clock::now();
	std::chrono::duration<double> d_diff = _end_time - _start_time;
	m_tool->time_diff = rcMax(m_tool->time_diff, d_diff.count() * 1000);
}
